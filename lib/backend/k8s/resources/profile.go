...
func (c *profileClient) getServiceAccount(ctx context.Context, rk model.ResourceKey, revision string) (*model.KVPair, error) {
	...
	serviceAccount, err := c.clientSet.CoreV1().ServiceAccounts(namespace).Get(context.Background(), serviceAccountName, metav1.GetOptions{ResourceVersion: revision})
	if err != nil {
...
func (c *profileClient) getNamespace(ctx context.Context, rk model.ResourceKey, revision string) (*model.KVPair, error) {
	...
	namespace, err := c.clientSet.CoreV1().Namespaces().Get(context.Background(), namespaceName, metav1.GetOptions{ResourceVersion: revision})
	if err != nil {
		return nil, K8sErrorToCalico(err, rk)
	}
...
func (c *profileClient) List(ctx context.Context, list model.ListInterface, revision string) (*model.KVPairList, error) {
	...
	namespaces, err := c.clientSet.CoreV1().Namespaces().List(context.Background(), metav1.ListOptions{ResourceVersion: nsRev})
	if err != nil {
	...
	serviceaccounts, err = c.clientSet.CoreV1().ServiceAccounts(kapiv1.NamespaceAll).List(context.Background(), metav1.ListOptions{ResourceVersion: saRev})
	if err != nil {
		return nil, K8sErrorToCalico(err, nl)
	}
...
func (c *profileClient) Watch(ctx context.Context, list model.ListInterface, revision string) (api.WatchInterface, error) {
	...
	if watchNS {
		...
		nsWatch, err = c.clientSet.CoreV1().Namespaces().Watch(context.Background(), opts)
		if err != nil {
	...
	if watchSA {
		...
		saWatch, err = c.clientSet.CoreV1().ServiceAccounts(ns).Watch(context.Background(), opts)
		if err != nil {
			nsWatch.Stop()
			return nil, K8sErrorToCalico(err, list)
		}
